...
def write_script(base_file, points, origin_z, z_length, script_path, save_path):
    text = """
print('start script')
DocumentOpen.Execute(r"{base_file}.scdoc")

# create datum plane
point = Point.Create(MM(0), MM(0), MM({origin_z}))
...
start = Point2D.Create(MM({s_point1_x}), MM({s_point1_y}))
end = Point2D.Create(MM({e_point1_x}), MM({e_point1_y}))
result = SketchLine.Create(start, end)

# draw second vertical line
start = Point2D.Create(MM({s_point2_x}), MM({s_point2_y}))
end = Point2D.Create(MM({e_point2_x}), MM({e_point2_y}))
result = SketchLine.Create(start, end)

# draw up line
start = Point2D.Create(MM({s_point1_x}), MM({s_point1_y}))
end = Point2D.Create(MM({s_point2_x}), MM({s_point2_y}))
result = SketchLine.Create(start, end)

# draw middle spline
points = List[Point2D]()
point_list = {spline_points}
for i in point_list:
    # print(i)
...
Body1 = ExtrudeFaces.Execute(selection, MM({z_length}), options)
selection = Selection.Create(GetRootPart().Bodies[0])
RenameObject.Execute(selection,"shape")

# Make Components
selection =Selection.CreateByNames("shape")
...
""" .format(s_point1_x=points[0][0], s_point1_y=points[0][1], e_point1_x=points[1][0], e_point1_y=points[1][1],
            s_point2_x=points[-1][0], s_point2_y=points[-1][1], e_point2_x=points[-2][0], e_point2_y=points[-2][1],
            spline_points=points[1:-1], origin_z=origin_z, z_length=z_length,
...
for i in range(1, opt_iter+1):
    ...
    project_address = r'G:\test\auto_diffuser\ad_v2'
    project_title = 'ad'
    version_name = 'V%s' % i
    line_number = 21
    ...
    if i == 1:
        ...
        new_thickness = process.single_thickness(boundary_list, original_thickness_list, target_velocity, np_file)
        x_list = shape_line.get_x(new_thickness)
        points = shape_line.get_points(x_list, line_number)

    # write SCDM script
    write_script(base_file_path, points, evap.oz, evap.lz, script_path, save_path)
